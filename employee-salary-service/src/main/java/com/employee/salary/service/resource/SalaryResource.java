...
import com.employee.salary.service.dao.PaySlipDao;
...
public class SalaryResource {
	...
	public ResponseEntity<CommonApiResponse> addSalaryPaySlipForUser(PaySlip paySlip) {

		CommonApiResponse response = new CommonApiResponse();

		if (paySlip == null) {
			response.setResponseMessage("Payslip is null");
			response.setSuccess(true);

			return new ResponseEntity<CommonApiResponse>(response, HttpStatus.BAD_REQUEST);
		}
		
		if(paySlip.getSalaryId() <= 0) {
			response.setResponseMessage("Select Salary to add PaySlip!!!");
			response.setSuccess(true);

			return new ResponseEntity<CommonApiResponse>(response, HttpStatus.BAD_REQUEST);
		}

		Salary salary = salaryService.getSalaryById(paySlip.getSalaryId());
		paySlip.setSalary(salary);
		PaySlip addedPaySlip = this.paySlipDao.save(paySlip);

		if (addedPaySlip == null) {
			response.setResponseMessage("Payslip not added!!!");
			response.setSuccess(true);

			return new ResponseEntity<CommonApiResponse>(response, HttpStatus.BAD_REQUEST);
		}

		response.setResponseMessage("PaySlip added Successfully");
		response.setSuccess(true);
	...
	public ResponseEntity<SalaryResponseDto> fetchSalaryPaySlipDetailsUsingUserId(int userId) {
		...
		paySlips = this.paySlipDao.findByUserId(userId);
		
		response.setPaySlips(paySlips);
		response.setResponseMessage("Pay Slips Fetched Successfully");
		response.setSuccess(true);
